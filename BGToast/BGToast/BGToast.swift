...
public extension UIView {
    ...
    private struct ToastKeys {
        static var timer        = "com.bgtoast.timer"
        static var duration     = "com.bgtoast.duration"
        static var point        = "com.bgtoast.point"
        static var completion   = "com.bgtoast.completion"
        static var activeToasts = "com.bgtoast.activeToasts"
        static var activityView = "com.bgtoast.activityView"
        static var queue        = "com.bgtoast.queue"
    }
    ...
    ///   - style: 样式
    ///   - completion: 状态
    public func makeToast(_ message: String?, duration: TimeInterval = BGToastManager.shared.duration, position: BGToastPosition = BGToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: BGToastStyle = BGToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
    ...
    ///   - style: 样式
    ///   - completion: 状态
    public func makeToast(_ message: String?, duration: TimeInterval = BGToastManager.shared.duration, point: CGPoint, title: String?, image: UIImage?, style: BGToastStyle = BGToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)?) {
        do {
            let toast = try toastViewForMessage(message, title: title, image: image, style: style)
     ...
     */
    public func showToast(_ toast: UIView, duration: TimeInterval = BGToastManager.shared.duration, position: BGToastPosition = BGToastManager.shared.position, completion: ((_ didTap: Bool) -> Void)? = nil) {
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    public func showToast(_ toast: UIView, duration: TimeInterval = BGToastManager.shared.duration, point: CGPoint, completion: ((_ didTap: Bool) -> Void)? = nil) {
        objc_setAssociatedObject(toast, &ToastKeys.completion, ToastCompletionWrapper(completion), .OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        
        if BGToastManager.shared.isQueueEnabled, activeToasts.count > 0 {
    ...
    ///
    /// - Parameters:
    ///   - includeActivity: <#includeActivity description#>
    ///   - clearQueue: <#clearQueue description#>
    ...
    /**
     ...
     @param position The toast's position
     */
    public func makeToastActivity(_ position: BGToastPosition) {
        // sanity
    ...
    public func hideToastActivity() {
        if let toast = objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView {
            UIView.animate(withDuration: BGToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseIn, .beginFromCurrentState], animations: {
                toast.alpha = 0.0
            }) { _ in
    ...
    private func makeToastActivity(_ toast: UIView, point: CGPoint) {
        ...
        UIView.animate(withDuration: BGToastManager.shared.style.fadeDuration, delay: 0.0, options: .curveEaseOut, animations: {
            toast.alpha = 1.0
        })
    }
    
    private func createToastActivityView() -> UIView {
        let style = BGToastManager.shared.style
    ...
    private func showToast(_ toast: UIView, duration: TimeInterval, point: CGPoint) {
        ...
        if BGToastManager.shared.isTapToDismissEnabled {
        ...
        UIView.animate(withDuration: BGToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseOut, .allowUserInteraction], animations: {
            toast.alpha = 1.0
        }) { _ in
     ...
     */
    public func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: BGToastStyle) throws -> UIView {
        // sanity
...
public enum BGToastPosition {
    case top
    case center
    case bottom
    
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        let topPadding: CGFloat = BGToastManager.shared.style.verticalPadding + superview.csSafeAreaInsets.top
        let bottomPadding: CGFloat = BGToastManager.shared.style.verticalPadding + superview.csSafeAreaInsets.bottom
